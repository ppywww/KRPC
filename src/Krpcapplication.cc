...
void KrpcApplication::Init(int argc, char **argv) {
    ...
    // 初始化日志系统
    std::string log_dir = m_config.Load("log_dir");
    GetInstance().logger_ = new KrpcLogger(argv[0], log_dir);
    
    // 记录初始化日志
    KRPC_LOG_INFO("===== Krpc 框架初始化成功 =====");
    KRPC_LOG_INFO("配置文件: " << config_file);
    if (!log_dir.empty()) {
        KRPC_LOG_INFO("日志目录: " << log_dir);
    } else {
        KRPC_LOG_INFO("日志输出: 控制台");
    }
}
...
// 析构函数定义
KrpcApplication::~KrpcApplication() {
    if (logger_) {
        delete logger_;
        logger_ = nullptr;
    }
