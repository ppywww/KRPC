...
void ZkClient::Create(const char *path, const char *data, int datalen, int state) {
    ...
    zoo_acreate(m_zhandle, path, data, datalen, &ZOO_OPEN_ACL_UNSAFE, state, 
        [](int rc, const char* path, const void* ctx) {
            SyncContext* sync_ctx = const_cast<SyncContext*>(static_cast<const SyncContext*>(ctx));
            {
                std::lock_guard<std::mutex> lock(sync_ctx->mutex);
                sync_ctx->rc = rc;
            }
            sync_ctx->cond.notify_one();
        }, &ctx);
    ...
    if (ctx.rc == ZOK) {  // 创建成功
        LOG(INFO) << "znode create success... path:" << path;
    } else if (ctx.rc == ZNODEEXISTS) {  // 节点已存在
...
std::string ZkClient::GetData(const char *path) {
    ...
    zoo_aget(m_zhandle, path, 0, 
        [](int rc, const char* data, int data_len, const struct Stat* stat, const void* ctx) {
            SyncContext* sync_ctx = const_cast<SyncContext*>(static_cast<const SyncContext*>(ctx));
            {
                std::lock_guard<std::mutex> lock(sync_ctx->mutex);
                sync_ctx->rc = rc;
                if (rc == ZOK && data_len > 0) {
                    sync_ctx->data = std::string(data, data_len);
                }
            }
            sync_ctx->cond.notify_one();
        }, &ctx);
